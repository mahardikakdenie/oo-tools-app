...
export default {
	...
	methods: {
		...
		isMenuActive(item) {
			return this.$route.path?.includes(item?.link) ? 'menu-item-active' : '';
		},
	},
...
.single-sidebar-menu {
	@apply relative;
	.menulabel {
		@apply text-slate-800 dark:text-slate-300 text-[10px] font-semibold uppercase mb-4 mt-4;
	}
	> .menu-link {
		@apply flex text-slate-600 font-medium dark:text-slate-300 text-xs capitalize px-[10px] py-3 rounded-[4px] cursor-pointer;
	}
	.menu-icon {
...
.menu-item-active {
	.menu-link {
		@apply bg-gradient-to-r from-blue-500 to-blue-700 dark:from-slate-700 dark:to-slate-900 shadow-lg rounded-lg transition-all duration-300 ease-in-out transform hover:scale-105;
		.icon-box,
		.menu-icon,
		.text-box {
			@apply text-white dark:text-yellow-400;
		}
	}
}
</style>
